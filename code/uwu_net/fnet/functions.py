...
def load_spatial_model(model, path_spatial_dir, gpu_ids=0):
    assert os.path.isdir(path_spatial_dir)
    path_spatial_model = os.path.join(path_spatial_dir, 'model.p')
    model.load_spatial(path_spatial_model, gpu_ids=gpu_ids)
    return model